...
def role_required(allowed_roles):
    ...
    """
    def decorator(view_func):  # الدالة التي تغلف الـ View
        @wraps(view_func)  # الحفاظ على خصائص الدالة الأصلية (مثل اسمها ووصفها)
        def _wrapped_view(request, *args, **kwargs):  # الدالة التنفيذية التي تفحص الصلاحيات
            ...
            if request.user.is_admin or request.user.role.name in allowed_roles:  # منطق الفحص المزدوج
                return view_func(request, *args, **kwargs)  # تمرير الطلب للدالة البرمجية في حال النجاح
            
            # في حال عدم توفر الصلاحية، يتم عرض رسالة خطأ ومنع الوصول
            messages.error(request, "ليس لديك صلاحية للوصول إلى هذه الصفحة.")  # إشعار المستخدم بالرفض
            return HttpResponseForbidden()  # إرجاع رد بروتوكول HTTP 403 (محظور)
